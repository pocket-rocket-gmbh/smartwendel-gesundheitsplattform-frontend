<template>
	<div class="box flex-column is-dark-grey pa-5">
		...
		<div class="py-3 d-flex flex-column justify-center" v-if="careFacility?.kind !== 'facility'">
			<span class="d-flex align-center">
				...
				<a
					...
					target="blank"
				>
					{{ careFacility?.user_care_facility.name }}
				</a>
			</span>
		</div>
		...
		<div v-if="careFacility?.email" class="py-3">
			<span class="d-flex align-center">
				...
				<a class="is-dark-grey general-font-size" :href="`mailto:${careFacility.email}`">
					{{ careFacility.email }}
				</a>
			</span>
		</div>
		...
		<div class="mt-3" v-if="careFacility?.kind === 'facility' && careFacility.opening_hours">
			<div style="display: flex; flex-direction: row; align-items: baseline">
				<div class="is-primary-color">
					<img class="mr-2 icon" :src="iconClock" />
				</div>
				<table style="width: 100%">
					<tr v-for="opening in careFacility.opening_hours" :key="opening.day">
						<td width="50%">{{ opening.day }}</td>
						<td width="50%">
							<span v-if="opening.hours">
								{{ opening.hours }}
							</span>
...
import iconClock from '@/assets/icons/facilities/icon_clock.svg';
