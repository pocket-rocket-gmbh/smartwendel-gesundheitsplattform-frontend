    ...
    <v-dialog v-model="dialog" fullscreen :scrim="false" transition="dialog-bottom-transition">
      <template v-slot:activator="{ props }">
        <div class="field" v-bind="props" @click="handleClearTermSearch()">
          ...
          <div class="input">{{ getAllSelectedCommunitiesName(filterStore.currentZips) || "Gemeinde wählen" }}</div>
        </div>
      </template>
      <v-card>
...
const handleOptionSelectCommunity = (community: any) => {
  if (filterStore.currentZips.includes(community?.zip)) {
    filterStore.currentZips = filterStore.currentZips.filter((item: any) => item !== community.zip);
  } else {
