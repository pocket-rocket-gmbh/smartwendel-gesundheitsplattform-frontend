				...
				<v-col md="9">
					<div class="py-10">
						<span class="general-font-size is-dark-grey facility-kind-description">
							Als Gesundheitsakteur im Landkreis St. Wunsiedel kannst du hier dein spezifisches
							Leistungsangebot in einem eigenen Profil darstellen und veröffentlichen. Fülle die
							Details zu deinem Angebot aus. Je spezifischer deine Angaben sind, desto besser können
							dich Besucher der Webseite finden. Pflichtfelder sind mit einem Sternchen versehen.
						</span>
					</div>
					...
					<div class="field" v-if="slotProps.item?.user?.onboarding_token">
						<div class="my-2 d-flex align-center">
							<span class="general-font-size is-dark-grey font-weight-bold mr-3">
								URL zur Einrichtungsübernahme generieren
							</span>
						</div>
					...
					<v-divider class="my-2" v-if="slotProps.item?.user?.onboarding_token?.length"></v-divider>
							...
							<div class="field split">
								...
								<v-form v-model="isFormValid">
									<v-text-field
										...
										hide-details="auto"
										:error-messages="useErrors().checkAndMapErrors('slug', slotProps.errors)"
							...
							<ClientOnly>
								...
								>
									<QuillEditor
										...
										@ready="onQuillReady"
									/>
									<div v-if="isDescriptionEmpty(slotProps.item.description)" class="required">
										Pflichtangabe
									</div>
									<v-text-field
										v-show="false"
					...
					>
						<span
							v-if="
								adressChanged && user.currentUser.is_active_on_health_scope && slotProps.item.id
							"
						>
							<v-alert type="warning" density="compact" class="mt-2">
								Es wurden Änderungen vorgenommen! Aufgrund dessen muss die Einrichtung erneut von
								einem Mitarbeiter verifiziert werden. Vielen Dank für deine Geduld!
							</v-alert>
						</span>
						<div class="my-2 d-flex align-center">
							<span class="general-font-size is-dark-grey font-weight-bold">
							...
							<div v-if="!slotProps.item.street || !slotProps.item.town">
								<div class="mt-3">
									<v-alert type="info" class="general-font-size">
										Adresse bitte vervollständigen.
									</v-alert>
								</div>
							</div>
							<div v-else-if="!isValidAddress && isValidAddress !== null && !loadingAdress">
								<div class="mt-3">
									<v-alert type="error" class="general-font-size">
										Adresse nicht gefunden. überprüfe deine Straße, Hausnr., Gemeinde und Ort
									</v-alert>
								</div>
							</div>
							<div class="mt-3" v-if="isValidAddress === null && editInformations">
								<v-alert type="info" class="general-font-size">
									Neue Adresse bitte eingeben.
								</v-alert>
							</div>
							<div
								v-if="
									isValidAddress && !loadingAdress && slotProps.item.street && slotProps.item.town
								"
							>
								<div class="mt-3">
									<v-alert type="success" class="general-font-size">
										Deine Adresse wurde erfolgreich hinterlegt. Bitte beachte, dass diese Adresse
										auch auf der Webseite angezeigt wird.
									</v-alert>
								</div>
							</div>
						</div>
...
const getLatLngFromAddress = async (zipCode: string, street: string, town: string) => {
	try {
		const { data } = await axios.get(
			`https://geocode.maps.co/search?postalcode=${zipCode}&street=${street}&country=DE&city=${town}&api_key=${config.public.GEOCODE_API_KEY}`
		);
		if (!data.length) {
