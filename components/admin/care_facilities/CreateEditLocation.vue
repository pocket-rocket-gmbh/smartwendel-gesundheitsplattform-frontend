<template>
	...
	>
		<v-card class="">
			<v-card-title v-if="location" class="general-font-size is-dark-grey">
				Ort bearbeiten
			</v-card-title>
...
const getLatLngFromAddress = async (zipCode: string, street: string) => {
	try {
		const { data } = await axios.get(
			`https://geocode.maps.co/search?postalcode=${zipCode}&street=${street}&country=DE&api_key=${config.public.GEOCODE_API_KEY}`
		);
...
const getAddressInfoFromLatLong = async (lat: number, long: number) => {
	const { data } = await axios.get(
		`https://geocode.maps.co/reverse?lat=${lat}&lon=${long}&api_key=${config.public.GEOCODE_API_KEY}`
	);
