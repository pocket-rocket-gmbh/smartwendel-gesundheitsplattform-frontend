				...
				<div class="list-item" :id="element.id" v-auto-animate>
						...
						<div class="title" v-auto-animate>
							<template v-if="openEdit !== element.id">
								<span
									class="item-title"
									:class="[
										element.id === expandCategory && element.next
								...
								>
									{{ element.title }}
									<v-icon v-if="element.next">mdi-arrow-down-right</v-icon>
								</span>
							</template>
							<template v-else>
						...
						<div v-if="!element.static" class="actions" v-auto-animate>
							<v-tooltip top v-if="openEdit !== element.id">
								<template v-slot:activator="{ props }">
									<v-icon class="clickable" v-bind="props" @click.stop="editClick(element)">
										mdi-pencil
									</v-icon>
								</template>
							...
							<v-tooltip top v-if="openEdit !== element.id">
								<template v-slot:activator="{ props }">
									<v-icon class="clickable" v-bind="props" @click.stop="deleteClick(element)">
										mdi-delete
									</v-icon>
								</template>
								<span>Löschen</span>
							...
							<v-tooltip top v-else>
								<template v-slot:activator="{ props }">
									<v-icon class="clickable" v-bind="props" @click.stop="discard">
										mdi-file-undo
									</v-icon>
								</template>
								<span>Verwerfen</span>
					...
					<div class="content" v-if="element.id === expandCategory && element.next">
						<CollapsibleListRec
							...
							"
							@edit-click="
								(action, prevItems, originalStep) =>
									handleEditEmit(action, [...prevItems, element.id], originalStep)
							"
							@entry-moved="
