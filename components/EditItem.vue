  ...
  >
    <v-card class="dialog">
      <v-card-title class="text-h5"
        ...
        ><span v-else>Adresse</span> ändern
      </v-card-title>
      <v-card-text v-if="type === 'email'"
        >Bitte beachten: Wenn du die E-Mail Adresse änderst, wird dein Benutzer wieder
      ...
      <v-card-actions>
        <v-row>
          ...
          <v-col md="6" class="d-flex justify-end">
            <v-btn variant="outlined" color="error" elevation="0" @click="emitAccept()">
              <span v-if="type === 'email'">E-Mail Adresse</span
              ><span v-if="type === 'slug'">Link</span
              ><span v-else>Adresse</span>ändern</v-btn
            >
          </v-col>
        </v-row>
      </v-card-actions>
