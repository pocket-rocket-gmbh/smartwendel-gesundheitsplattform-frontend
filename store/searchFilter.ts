	...
	actions: {
		...
		async loadAllFacilityFilters() {
			...
			const relevantFilter = filters.find(
				(filter) => filter.filter_type === 'filter_facility' && filter.kind === this.currentKinds[0]
			);
			...
			this.allFacilityMainFilters.forEach((mainFilter) => {
				mainFilter.options =
					filterOptions.find((filterOption) => filterOption.parentId === mainFilter.id)?.options ||
					[];
			});
			...
			this.filteredCommunities = this.allCommunities
				...
				.filter((community) => {
					...
					return this.filteredResults.find(
						(result) =>
							result.community_id === community.id || this.currentZips.includes(result.zip)
					);
				});
						...
						(!this.onlySearchInTitle &&
							(result.description?.toUpperCase().includes(this.currentSearchTerm.toUpperCase()) ||
								result.tags.find((tag) =>
									tag.name.toUpperCase().includes(this.currentSearchTerm.toUpperCase())
								)))
					);
				})
				.filter((result) => {
