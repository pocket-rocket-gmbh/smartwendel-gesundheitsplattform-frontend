      ...
      <v-card  :class="['pa-6', {'shake' : animated}]">
        ...
        <div v-if="userId">
          ...
          <div class="mb-2 mt-2">Bitte ändere Dein Passwort</div>
            ...
            <div class="mt-2" v-if="password.length < 5 || buttonDisabled">Das Passwort muss mindestens 5 Zeichen lang sein. Beide Passwörter müssen übereinstimmen.</div>
          </div>
        </div>
...
const buttonDisabled = computed(() => {
  return !(password.value.length > 4 && password_confirmation.value.length > 4 && password.value === password_confirmation.value)
})
